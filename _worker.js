    ...
    if (isArticle && isCrawler) {
      ...
      try {
        ...
        if (fsRes.ok) {
          ...
          const rawCover  = fields.coverImage?.stringValue || '';
          // Reject placeholder/invalid image sources, fall back to real OG image
          const cover     = (rawCover && !rawCover.includes('picsum') && !rawCover.includes('placeholder') && !rawCover.includes('lorem')) ? rawCover : DEFAULT_IMAGE;
